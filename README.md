# 🎬 Netflix Analytics Dashboard

Welcome to the **Netflix Analytics Dashboard**!  
This Power BI dashboard provides clear insights into Netflix’s 2023 catalog, including movie trends, durations, and global content distribution.




---

##📁 Repository Structure

1. Netflix-Analytics
2. data — dataset (Netflix dataset)
3. documentation — analysis notes and explanations
4. README.md — main project description
5. LICENSE — license file

---






---

## 🌟 Pages Overview

- **Page 1 — Movies**: Summary of Netflix movies with trends, ratings, and content counts.  
- **Page 2 — Duration Analysis**: Explore runtimes of movies and TV shows over time.  
- **Page 3 — Geography Insights**: Analyze content distribution across countries, top 10 countries, and genre trends.

---

## 🔸 Page 1 — Movies

Page 1 focuses on Netflix movies, showing counts, ratings, and trends.

<p align="center">
  <img src="https://github.com/GunelXelilli/Netflix-Analytics-Dashboard/blob/main/Netflix1page.png" width="800">
</p>


---

## 🔸 Page 2 — Duration Analysis

Page 2 visualizes runtime patterns for movies and TV shows.

<p align="center">
  <img src="https://github.com/GunelXelilli/Netflix-Analytics-Dashboard/blob/main/Netflixpage2.png" width="800">
</p>


---

## 🔸 Page 3 — Geography Insights

Page 3 explores Netflix content across countries and genres.

<p align="center">
  <img src="https://github.com/GunelXelilli/Netflix-Analytics-Dashboard/blob/main/Ntflix3page.png" width="800">
</p>


📫 **Contact**

- 🧑‍💻 GitHub: [GunelXelilli](https://github.com/GunelXelilli)  
- 🐘 LeetCode: [Gunel87](https://leetcode.com/u/Gunel87/)  
- 💼 LinkedIn: [Gunel Xelilli](https://www.linkedin.com/in/gunel-xelilli-390bb32b7/)


---

## 🛠️ How to Explore

1. View screenshots in the `README.md` folder.  
2. Use Netflix sample data in `data/` to recreate or test the dashboard.  
3. Open Power BI Desktop to explore the dashboard interactively.

--- 

*Created with ❤️ and Power BI skills!* 🎨


